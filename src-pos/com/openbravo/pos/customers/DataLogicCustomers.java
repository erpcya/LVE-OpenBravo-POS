...
public class DataLogicCustomers extends BeanFactoryDataSingle {
    ...
    //	Dixon Martinez
    //	18/04/2014 Add Support For Localization Customers
    
    /**
     * Return Country Information by C_Country_ID
     ...
     */
    public C_CountryInfo getC_Country(String p_C_Country_ID) throws BasicException{
    	String sql = "SELECT "
    			+ "	C_Country_ID, " //	1
    			+ " IsActive, " 	//	2
    			+ "	Name, "			//	3
    	...
    	return (C_CountryInfo) 
    			new PreparedSentence(
    					s, 
    					sql, 
    					SerializerWriteString.INSTANCE, 
    					C_CountryInfo.getSerializerRead()
    					).find(p_C_Country_ID);
     ...
     * @author Dixon Martinez
     * 18/04/2014
     * @return List(C_CountryInfo> 
     */
    public final List<C_CountryInfo> getC_Countrys(){
    	String sql = "SELECT "
    			+ "	IsActive, " //	1
    			+ " C_Country_ID," 	//	2
    			+ "	Name, "			//	3
    	...
    	try {
			...
			e.printStackTrace();
			return new ArrayList<C_CountryInfo>(){};
		}
    	
    }

	public final List<C_RegionInfo> getC_Regions(String p_C_Country_ID) {
		if(p_C_Country_ID == null)
			return 
					new ArrayList<C_RegionInfo>(){};
		String sql = " SELECT "
				+ "	C_Region_ID,"
				+ "	 C_Country_ID, "
				+ "	Name,"
		...
		try {
			return 
					new PreparedSentence(
							...
							sql,
							SerializerWriteString.INSTANCE,
							C_RegionInfo.getSerializerRead()).list(p_C_Country_ID);
		} catch (BasicException e) {
				e.printStackTrace();
			return 
					new ArrayList<C_RegionInfo>(){};
	...
	public final List<C_CityInfo> getC_City(String p_C_Region_ID) {
		if(p_C_Region_ID == null)
			return 
					new ArrayList<C_CityInfo>(){};
					
		System.out.println("Region_ID = "+ p_C_Region_ID);
		String sql = "SELECT C_City_ID, Name, C_Region_ID,  AreaCode, Postal, IsActive"
				+ " FROM C_City"
				+ " WHERE"
				+ " 	C_Region_ID = ?"
				+ "		AND IsActive = 'Y'";
		...
		try {
			return 
					new PreparedSentence(
								...
								sql,
								SerializerWriteString.INSTANCE,
								C_CityInfo.getSerializerRead()).list(p_C_Region_ID);
		} catch (BasicException e) {
				e.printStackTrace();
			return 
					new ArrayList<C_CityInfo>(){};
